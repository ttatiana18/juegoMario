...
if __name__ == '__main__': 
	...
	all_sprites, all_bloques, all_generadores_caracoles,all_plantas_enemies = obj_lectura_mapa.cargarObjetosMapa(1)

	fondo=pygame.image.load('./data/img/mapa3.png')
	fondo2=pygame.image.load('./data/img/mapafinal.png')
	...
	while not fin_juego and not fin:
		...
		for bloque_e in all_bloques:
			if bloque_e.tipo_b==1:
				...
				if bloque_e.temp<0:
					if bloque_e.cont<10:
						hongo=Hongo_enemigo(all_bloques,[bloque_e.rect.x,bloque_e.rect.y],1)   
						hongo.vel_x=2             
	...
	if (jugador.vida<=0 or condicion3) or (tiempo<=0):
		...
		all_sprites, all_bloques, all_generadores_caracoles,all_plantas_enemies = obj_lectura_mapa.cargarObjetosMapa(2)
		info2=fondo2.get_rect()
		fondo2_ancho=info2[2]
		fondo2_alto=info2[3]
		f_x = 0
		f_y = 0
		all_modificadores=pygame.sprite.Group()
		...
		while not fin_juego and not fin:
			...
			for bloque_e in all_bloques:
				if bloque_e.tipo_b==1:
					...
					if bloque_e.temp<0:
						if bloque_e.cont<10:
							hongo=Hongo_enemigo(all_bloques,[bloque_e.rect.x,bloque_e.rect.y],2)   
							hongo.vel_x=2             
							all_enemies.add(hongo)
							all_sprites.add(hongo)
							bloque_e.temp=400
			...
			pantalla.blit(fondo2,[ jugador.f_x , jugador.f_y])
			pantalla.blit(texto,[450,50])
			pantalla.blit(texto2,[800,50])
			all_sprites.update()
			all_sprites.draw(pantalla)
